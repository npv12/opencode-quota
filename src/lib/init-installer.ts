...
const QUOTA_PLUGIN_SPEC = "@npv12/opencode-quota@latest";
  ...
  const spec =
    typeof entry === "string"
      ...
      : Array.isArray(entry) && typeof entry[0] === "string"
        ? entry[0]
        : undefined;
  if (spec === "@npv12/opencode-quota") return spec;
  if (
    spec !== undefined &&
    /^@npv12\/opencode-quota@(?:v?\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?|[A-Za-z][0-9A-Za-z._-]*)$/.test(
      spec,
    )
  ) {
    return spec;
...
async function promptForSelections(
  ...
  prompts.log.info("npx @npv12/opencode-quota@latest provider add");
